...
class DBDriver {
  ...
  Desire noteFromMap(Map map) => Desire(
      id: map['id'],
      title: map['title']
  );

  Map<String, dynamic> noteToMap(Desire desire) => <String, dynamic>{
    'id' : desire.id,
    'title' : desire.title
  };
  ...
  Future<List<Desire>> getAll() async {
    ...
    return desires.isNotEmpty ? desires.map((e) => noteFromMap(e)).toList() : [];
  }

  Future<void> update(Desire desire) async {
    Database db = await database;

    return await db.update('desires', noteToMap(desire), where: 'id = ?', whereArgs: [desire.id]);
  }
